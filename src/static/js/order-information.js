...
const addBasicInfo = () => {
  ...
  $(".basic-info-widget .time-of-purchase").text(
    moment(window.order.timestamp).format("MMM DD, YYYY LT")
  );
};

const addCustomerDetails = () => {
  $(".customer-details-widget .first-name").text(
    `${window.order.first_name || "None"}`
  );
  $(".customer-details-widget .last-name").text(
    `${window.order.last_name || "None"}`
  );
  $(".customer-details-widget .email").text(`${window.order.email || "None"}`);
  $(".customer-details-widget .phone-number").text(
    `${window.order.phone_number || "None"}`
  );
  $(".customer-details-widget .address-line-one").text(
    `${window.order.address_line_one || "None"}`
  );
  $(".customer-details-widget .address-line-two").text(
    `${window.order.address_line_two || "None"}`
  );
  $(".customer-details-widget .city").text(`${window.order.city || "None"}`);
  $(".customer-details-widget .state").text(`${window.order.state || "None"}`);
  $(".customer-details-widget .zip-code").text(
    `${window.order.zip_code || "None"}`
  );
};
...
const addSeatInfo = () => {
  ...
  window.order.seats.forEach(item => {
    ...
    $(".seat-info-widget .seats-wrap").append(
      `<div class="seat-wrap"><div class="seat">${vipIcon}</div><div class="seat-info"><p class="seat-number">Seat ${item.name} - <span class="seat-status">${item.type}</span></p><p class="seat-name">${item.attendee_name}</p></div><p class="seat-price">$${item.price}</p></div>`
    );
  });

  $(".seat-info-widget .subtotal-wrap .charge").text(
    price.subTotal === 0 ? "Free" : `$${price.subTotal.toFixed(2)}`
  );
  $(".seat-info-widget .fees-wrap .charge").text(
