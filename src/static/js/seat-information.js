const appendInputs = () => {
  ...
  seats.forEach(item => {
    ...
    $(`.form-content ${appendWrap} .inputs-wrap`).append(
      `<div class="seat-input-wrap" data-seat-id="${item.seat_id}" data-seat="${item.name}"><div class="seat-number-wrap"><p class="seat-number">${item.name}</p></div><input value="${inputVal}" class="seat-name-inpt" type="text" placeholder="First and Last Name" onkeypress="return (event.charCode >= 97 && event.charCode <= 122) || (event.charCode >= 65 && event.charCode <= 90) || event.charCode == 32"/></div>`
    );
  });
