...
const setPrice = change => {
  ...
  if (overview.price <= 0) {
    ...
    $(".purchase-wrap p.total").html(
      `Total: <span class="total-price">$${overview.price}+ Fees</span>`
    );
  }
};

const addSeatInfo = (seat, price) => {
  $(".overview-wrap .seats-wrap").append(
    `<div class="seat-number-wrap"><p class="seat-price">$${price}</p><p class="seat-number">${seat}</p></div>`
  );
};
...
const selectSeat = seat => {
  ...
  if (seat.hasClass("vip-seat")) {
    number =
      parseInt($(".vip-tickets-wrap .number-of-tickets .number").text()) + 1;
    ...
    number =
      parseInt($(".ga-tickets-wrap .number-of-tickets .number").text()) + 1;
